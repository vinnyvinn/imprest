            ...
            <div class="col-md-10">
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        ...
                        <a href="{{route('imprest.myunprocessed')}}" class="href btn btn-success btn-xs">My Imprests</a>
                        @endif
                    ...
                    <div class="panel-body">
                        <table class="table table-responsive table-striped table-hover" id="datatbl">
                            <thead>
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <!-- <a target="_blank" href="//{{ route('imprest.print', $imprest->id) }}" class="btn btn-success btn-xs">Print</a> -->
                                        @if($imprest->status == App\Imprest::STATUS_UNPROCESSED && $imprest->applicant_id == Auth::user()->sage_id )
                                            <label class="label label-info">  HOD  Pending Approval</label>
                                            <label class="label label-info">  Admin Pending Approval</label>
                                            <label class="label label-info">  Finance Pending Approval</label>

                                            @elseif($imprest->status == App\Imprest::STATUS_APPROVED && $imprest->applicant_id == Auth::user()->sage_id )
                                                <label class="label label-success">  HOD Approved</label>
                                                <label class="label label-info">  Admin Pending Approval</label>
                                                <label class="label label-info">  Finance Pending Approval</label>
                                        ...
                                        @elseif($imprest->status == 2 && Auth::user()->role_id==3 && $imprest->applicant_id != Auth::user()->sage_id || $imprest->currency ==1)

                                            <a href="{{ route('imprest.edit', $imprest->id) }}" class="btn btn-success btn-xs">Action</a>

                                        @elseif($imprest->status == 2 && Auth::user()->role_id==0 && $imprest->applicant_id != Auth::user()->sage_id)

                                            <a href="{{ route('imprest.edit', $imprest->id) }}" class="btn btn-success btn-xs">Action</a>

                                        @elseif($imprest->status == 2 && Auth::user()->role_id==0 && $imprest->applicant_id != Auth::user()->sage_id && $imprest->currency==1)
                                        ...
                                        @elseif( Auth::user()->role_id !=1 && $imprest->status == 2 && (hasPermission(App\User::PERM_EDIT_IMPREST) || hasPermission(App\User::PERM_FINALIZE_IMPREST)) && $imprest->applicant_id != Auth::user()->sage_id)
                                            ...
                                            @endif
                                        @elseif($imprest->currency==1 && $imprest->status == App\Imprest::STATUS_APPROVED && (hasPermission(App\User::PERM_EDIT_IMPREST) || hasPermission(App\User::PERM_FINALIZE_IMPREST)) && $imprest->applicant_id != Auth::user()->sage_id)
                                            <a href="{{ route('imprest.edit', $imprest->id) }}" class="btn btn-success btn-xs">Action</a>

                                        {{--@elseif($imprest->status == 1 && (hasPermission(App\User::PERM_EDIT_SURRENDER_IMPREST) || hasPermission(App\User::PERM_FINALIZE_SURRENDER_IMPREST))--}}
                                        {{--&& $imprest->applicant_id != Auth::user()->sage_id)--}}
                                                  {{--<a href="{{ route('surrender.edit', $imprest->id) }}" class="btn btn-success btn-xs">Action2</a>--}}

                                        @elseif($imprest->status == App\Imprest::STATUS_UNPROCESSED && Auth::user()->role_id ==1)
                                            <label class="label label-success"> HOD pending

                                            </label>

                                        @elseif($imprest->status == 2 && Auth::user()->role_id ==0)
