...
@section('content')
    <div class="container">
        ...
        <form action="{{ url('makesurrender',['id' => $imprest->id]) }}" method="POST">
    <input type="hidden" name="_token" value="{{csrf_token()}}">
    {{ method_field('PUT') }}
    <input type="hidden" name="imprest_id" value="{{ $imprest->id }}">
                 	<div class="row">
						<div class="col-md-12">
							<table class="table table-stripped">
                                ...
                                @foreach($imprest->surrenderlines as $lines)
                                   <tr>
                                     ...
                                     <td>
                                         <select class="form-control select2" name="account[]" @if( ($lines->status ==App\SurrenderAttachment::HOD_CANCELLED) || ($lines->status ==App\SurrenderAttachment::FINAL_CANCELLED))
                                            readonly="true"
                                        @else
                 	...
                 	<div class="row">
                 		<div class="col-md-6 col-md-offset-3">
							<button type="submit" class="btn btn-sm btn-success"> Approve Request
                            </button>
